...
export default function SettingsPage() {
  ...
  const [orgInfo, setOrgInfo] = useState({ name: '', primary_color: '#1a365d' })
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const [settingsRes, integrationsRes, orgRes, usageRes] = await Promise.all([
        ...
        supabase.from('organizations').select('name, primary_color').eq('id', orgId).single(),
      ...
      if (orgRes.data) setOrgInfo({ name: orgRes.data.name, primary_color: orgRes.data.primary_color || '#1a365d' })
